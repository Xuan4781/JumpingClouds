...
while (to_move_y != 0) {
	...
	if (dir >= 0 && !was_hit_by_player) {
		...
		if (!colliding && !hit_this_frame) {
			collidewith = instance_place(x, y + dir, opponent_object); // Uses opponent_object
			if (collidewith != noone) {
...
if (x > room_width - half_w) { x = room_width - half_w; x_vel = -abs(x_vel); r_x = 0; audio_play_sound(so_wall, 1, false); }

// player movement
if (!was_hit_by_player) {
	if (keyboard_check(left_key)) x_vel -= accel;
	if (keyboard_check(right_key)) x_vel += accel;
}


//respawn
if (active) {
	// check if player collide particles
